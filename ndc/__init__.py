...
class NDC:
    ...
    * `image` is a path to a disk image.
    ...
    def __validate_bin_version(self):
        version = (
            subprocess.check_output(self.bin).decode(self.encoding).splitlines().pop(0)
        )
    ...
    def __run(self, *args):
        ...
        try:
            result = subprocess.check_output(cmd).decode(self.encoding).splitlines()
    ...
    def list(self, image, path="", partition=0) -> Tuple[str, str, datetime]:
        SKIP = [".", ".."]
        args = [
            expanduser(image),
            str(partition),
        ...
        return [self.__parse(args) for args in self.__run(*args) if args[0] not in SKIP]
